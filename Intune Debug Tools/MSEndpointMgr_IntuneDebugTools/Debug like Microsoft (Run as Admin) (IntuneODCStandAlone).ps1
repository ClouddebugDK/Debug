...
$xmlPath =  'C:\windows\temp\Intune.XML'
...
if (-not (Test-Path $xmlPath) ) {
    try {
       ...
       if ( $null = [System.Net.WebProxy]::GetDefaultProxy().Address   ) {
                Invoke-WebRequest -UseBasicParsing -Uri $downLoadLocation -OutFile $xmlPath
                }
            else {
    
                $myproxy = ([System.Net.WebProxy]::GetDefaultProxy().Address.AbsoluteURI)    
                Invoke-WebRequest -UseBasicParsing -Uri $downLoadLocation -OutFile $xmlPath -Proxy $myproxy
            }
...
if ( ( get-content $xmlPath -TotalCount 1) -ne '<?xml version="1.0" encoding="utf-8"?>' ){
